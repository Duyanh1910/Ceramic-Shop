...
export default function AdminReceivedNotes() {
  ...
  const detailColumns = [
    {
      ...
      render: (variant) => {
        ...
        const productText = getProductVariantText(variant);

        return (
          <div className={styles.receivedNoteProductCell}>
            {image ? (
              <Image
                src={image}
                alt={productText}
                className={styles.receivedNoteProductImage}
                preview={false}
              />
            ) : (
              <div className={styles.receivedNoteProductImagePlaceholder} />
            )}

            <Tooltip title={productText}>
              <div className={styles.receivedNoteProductName}>
                {productText}
              </div>
            </Tooltip>
          </div>
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "right",
      render: (value) => (
        <span className={styles.price}>{formatCurrency(value)}</span>
      ),
    },
    {
      title: "Trạng thái",
      ...
      >
        ...
        >
          ...
          >
            <Input.TextArea
              rows={2}
              placeholder="Ví dụ: Nhập lô hàng đầu tháng"
            />
          </Form.Item>
          ...
          <Form.List name="items">
            {(fields, { add, remove }) => (
              <>
                {fields.map((field) => {
                  const rowVariants =
                    variantOptionsByRow[String(field.name)] || [];
