...
const createMenuItems = (isAdmin) => [
  {
    key: "system",
    icon: <SettingOutlined />,
    label: "Quản lý hệ thống",
    children: isAdmin ? SYSTEM_MENU_ADMIN : SYSTEM_MENU_STAFF,
  },
  {
    key: "catalog",
    icon: <AppstoreOutlined />,
    label: "Quản lý danh mục",
    children: CATALOG_MENU,
  },
  {
    key: "sales",
    icon: <ShoppingOutlined />,
  ...
  },
  {
    key: "support",
    icon: <SafetyOutlined />,
    label: "Tư vấn, khuyến mãi, hậu mãi",
    children: SUPPORT_MENU,
  },
  ...(isAdmin
    ? [
        {
...
export default function AdminLayout() {
  ...
  useEffect(() => {
      ...
      notification[cfg.antType]({
        message: (
          <span style={{ fontWeight: 700, fontSize: 14 }}>
            {payload?.title || "Thông báo đơn hàng"}
          </span>
        ),
        description: (
    ...
    const handleAdminNotification = (payload) => {
      const notifType =
        payload?.type || payload?.LoaiThongBao || "ORDER_STATUS_UPDATED";
  ...
  const username =
    ...
    "Tài khoản";
  const menuItems = createMenuItems(isAdmin);
