...
const WarrantyHistory = ({ open, onCancel, maBaoHanh, onUpdated }) => {
  ...
  const fetchWarrantyDetail = async () => {
    ...
    try {
      ...
      message.error(err.response?.data?.message || "Không thể tải chi tiết bảo hành!");
    } finally {
      setLoading(false);
    }
  };
          ...
          <>
            ...
            >
              <Descriptions.Item label="Khách hàng">
                <Text strong>
                  {warrantyData.ChiTietDonHang?.DonHang?.TenNguoiNhan || "Không rõ"}
                </Text>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="Địa chỉ" span={2}>
                {warrantyData.ChiTietDonHang?.DonHang?.DiaChiGiaoHang || "Không rõ"}
              </Descriptions.Item>

              <Descriptions.Item label="Mã đơn hàng">
                <Text copyable className={styles.highlightText}>
                  {warrantyData.ChiTietDonHang?.DonHang?.MaHienThi || "Không rõ"}
                </Text>
              </Descriptions.Item>

              <Descriptions.Item label="Sản phẩm / Phân loại">
                <Text strong>
                  {warrantyData.ChiTietDonHang?.BienTheSanPham?.TenBienThe || "Không rõ"}
                </Text>
              </Descriptions.Item>
            ...
            <div className={styles.actionBar}>
              <Space wrap>
                ...
                {Number(warrantyData.TrangThai) === WARRANTY_STATUS.PROCESSING && (
                  <Button
                    type="primary"
                    loading={updating}
