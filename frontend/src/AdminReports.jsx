...
export default function AdminReports() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (resOverview.data?.success) {
        ...
        const rev = overviewData.summary.revenue;
        const ord = overviewData.summary.orders;
        
        setSummary({
          revenue: rev,
          orders: ord,
          customers: overviewData.summary.customers, 
          avgOrder: ord > 0 ? Math.floor(rev / ord) : 0,
        });
      }
  ...
  const bestSellerColumns = [
    ...
    { title: 'Sản phẩm', dataIndex: 'TenSanPham', render: (v) => <span className={styles.productName}>{v}</span> },
    { title: 'Đã bán', dataIndex: 'TongDaBan', render: (v) => <Tag color="volcano"><FireOutlined /> {v} SP</Tag> },
    { title: 'Tỉ trọng', dataIndex: 'percent', render: (v) => (
        <div className={styles.barWrap}>
          <div className={styles.barFill} style={{ width: `${v * 3}px` }} /><span>{v}%</span>
        </div>
      )
    },
  ];
  ...
  const mostViewedColumns = [
    ...
    { title: 'Sản phẩm', render: (record) => (
        <div className={styles.flexCenter}>
          ...
          <div><div className={styles.productName}>{record.TenSanPham}</div><div style={{ fontSize: 11, color: '#888' }}>{record.ThuongHieu}</div></div>
        </div>
      )
    },
    { title: 'Lượt xem', dataIndex: 'LuotXem', render: (v) => <Tag color="geekblue"><EyeOutlined /> {v}</Tag> }
  ];
  ...
  const ratingColumns = [
    ...
    { title: 'Điểm TB', dataIndex: 'DiemTrungBinh', render: (v) => <span className={styles.starText}><StarOutlined /> {Number(v).toFixed(1)}</span> },
    { title: 'Số lượt ĐG', dataIndex: 'TongDanhGia', render: (v) => <Tag color="green">{v}</Tag> }
  ];
    ...
    <div className={styles.wrapper}>
      <div className={styles.pageHeader}>
        ...
        <div className={styles.filters}>
          ...
          {mode === 'month' && <Select value={month} onChange={setMonth} className={styles.filterSelect}>{MONTHS.map(m => <Select.Option key={m.value} value={m.value}>{m.label}</Select.Option>)}</Select>}
          {mode === 'quarter' && <Select value={quarter} onChange={setQuarter} className={styles.filterSelect}>{QUARTERS.map(q => <Select.Option key={q.value} value={q.value}>{q.label}</Select.Option>)}</Select>}
          <Select value={year} onChange={setYear} className={styles.filterSelect}>{YEARS.map(y => <Select.Option key={y} value={y}>{y}</Select.Option>)}</Select>
        </div>
      </div>
        ...
        <>
          <Row gutter={[16, 16]} className={styles.summaryRow}>
            {[
              ...
              { title: 'Khách hàng', value: summary.customers, icon: <TeamOutlined />, color: '#c48c46', bg: '#fff8e6' },
              ...
              <Col xs={24} sm={12} xl={6} key={i}>
                <div className={styles.summaryCard}>
                  ...
                  <div><div className={styles.summaryLabel}>{card.title}</div><div className={styles.summaryValue} style={{ color: card.color }}>{card.value}</div></div>
                </div>
              </Col>
            ))}
          </Row>
          ...
          <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
            <Col xs={24} xl={16}>
              <div className={styles.chartCard}>
                <div className={styles.chartHeader}><span className={styles.chartTitle}>Doanh thu theo {periodLabel()}</span></div>
            ...
            <Col xs={24} xl={8}>
              <div className={styles.chartCard}>
                <div className={styles.chartHeader}><span className={styles.chartTitle}>Đơn hàng</span></div>
                <ResponsiveContainer width="100%" height={300}>
          ...
          <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
            <Col xs={24} xl={16}>
              <div className={styles.chartCard}>
                <div className={styles.chartHeader}><span className={styles.chartTitle}>Top sản phẩm bán chạy</span></div>
                <Table dataSource={bestSellers} columns={bestSellerColumns} rowKey="MaSanPham" pagination={false} size="small" className={styles.productTable} />
              </div>
            </Col>
            <Col xs={24} xl={8}>
              <div className={styles.chartCard}>
                <div className={styles.chartHeader}><span className={styles.chartTitle}>Tỉ trọng số lượng bán</span></div>
                {pieData.length > 0 ? (
                  <><ResponsiveContainer width="100%" height={260}>
                      <PieChart><Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={95} paddingAngle={3} dataKey="value" label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`} labelLine={false}>{pieData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip formatter={(v) => [`${v}%`, 'Tỉ trọng']} /></PieChart>
                    </ResponsiveContainer>
                    <div className={styles.legend}>{pieData.map((item, i) => (<div key={i} className={styles.legendItem}><span className={styles.legendDot} style={{ background: COLORS[i % COLORS.length] }} /><span className={styles.legendLabel}>{item.name}</span></div>))}</div>
                  </>
          ...
          <Row gutter={[16, 16]}>
            <Col xs={24} xl={12}>
              <div className={styles.chartCard}>
                <div className={styles.chartHeader}><span className={styles.chartTitle}>Sản phẩm xem nhiều nhất</span></div>
                <Table dataSource={mostViewed} columns={mostViewedColumns} rowKey="MaSanPham" pagination={false} size="small" className={styles.productTable} />
              </div>
            </Col>
            <Col xs={24} xl={12}>
              <div className={styles.chartCard}>
                <div className={styles.chartHeader}><span className={styles.chartTitle}>Top đánh giá cao</span></div>
                <Table dataSource={topRatings} columns={ratingColumns} rowKey="MaSanPham" pagination={false} size="small" className={styles.productTable} />
              </div>
            </Col>
          </Row>
        </>
