...
export const sendEmailInvoiceService = async (email, orderCode) => {
  try {
    ...
    let title = "Cảm ơn bạn đã tin chọn The Ceramic Shop!";
    ...
    const orderItemsHtml = chiTietDonHangs
      ...
      <tr>
        <td style="padding: 15px; border-bottom: 1px dashed #e0d5c1; color: #3e3222;">
          <strong style="font-size: 15px;">${item.Variant?.Product?.TenSanPham || item.BienTheSanPham?.SanPham?.TenSanPham || "Sản phẩm"}</strong> <br/>
          <span style="color: #8b7d6b; font-size: 13px;">Phân loại: ${item.Variant?.TenBienThe || item.BienTheSanPham?.TenBienThe || "Mặc định"}</span>
        ...
        <tr>
          <td align="center">
            <table width="680" border="0" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 8px 24px rgba(90, 75, 60, 0.08); margin: 0 auto; max-width: 680px; border: 1px solid #e0d5c1;">
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-collapse: collapse; margin-bottom: 20px; background-color: #ffffff;">
                    <thead>
                      <tr style="background-color: #f2ebe1;">
                        <th style="padding: 14px 15px; text-align: left; color: #5a4b3c; font-size: 13px; text-transform: uppercase; letter-spacing: 1px; border-top-left-radius: 6px; border-bottom-left-radius: 6px;">Sản phẩm</th>
                        ...
                        <th style="padding: 14px 15px; text-align: right; color: #5a4b3c; font-size: 13px; text-transform: uppercase; letter-spacing: 1px; border-top-right-radius: 6px; border-bottom-right-radius: 6px;">Thành tiền</th>
                      </tr>
                    </thead>
                    <tbody>
                      ${orderItemsHtml}
              ...
              <tr>
                <td align="center" style="padding: 30px 40px; background-color: #3e3222; border-bottom-left-radius: 12px; border-bottom-right-radius: 12px;">
